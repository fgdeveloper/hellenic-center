...
?>
    ...
    <div id="content">
        ...
        <?php 
            ...
            $blog_ex_categories = get_post_meta($post->ID, 'blog_ex_categories', true);
            
            
            // authors            
            $blog_authors = get_post_meta($post->ID, 'blog_authors', true);
            if(!is_array($blog_authors)) { $blog_authors = Array(); }
            $blog_authors_count = count($blog_authors);
            $blog_authors_arg = '';
            if($blog_authors_count > 0)
            {
                for($i = 0; $i < $blog_authors_count; $i++)
                {
                    if($i > 0) $blog_authors_arg .= ',';
                    $blog_authors_arg .= $blog_authors[$i];        
                }    
            }
            // excluded categories 
            $excluded = $dccp->getIGeneral()->getExcludedCategories();
            if($blog_ex_categories != '')
            {
                $excluded = explode(',', $blog_ex_categories);
                ...
                while(have_posts())
                {
                    ...
                    $out .= '<div class="blog-post">';   
                        ...
                        $out .= '<div class="info">
                                ...
                                __('Posted by', 'dc_theme').'&nbsp;<a href="'.get_author_posts_url($post->post_author).'" class="author">'.get_the_author_meta('display_name', $post->post_author).'</a> in ';
            ...
            {
                echo '<p>There are no posts in this blog.</p>';
            }   
