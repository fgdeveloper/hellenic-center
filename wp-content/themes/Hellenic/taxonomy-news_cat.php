...
* Brief:       
*      Theme single news category page code
* Author:      
*      DigitalCavalry
* Author URI:
...
***********************************************************************/ 
    ...
    $news_cat_slug = (get_query_var('news_cat')) ? get_query_var('news_cat') : '';
    $news_cat_info = get_term_by('slug', $news_cat_slug, 'news_cat', OBJECT); 
    
    $dccp = GetDCCPInterface();                                      
?>

    <div class="navigation-tree-container">        
        <?php dcf_naviTree($post->ID, 0, 'Archive for news '.$news_cat_info->name); ?> 
    </div>
    ...
    <div id="content">
        <?php
                ...
                echo '<h1>'.$news_cat_info->name.'</h1>';
                if($news_cat_info->description != '')
                {                        
                    echo '<div sytle="margin-bottom:20px;">'.do_shortcode($news_cat_info->description).'</div>';
                }
       
                $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;  
                $args = Array('paged'=>$paged, 'post_type' => 'news', 'taxonomy' => 'news_cat', 'term' => $news_cat_info->slug);
