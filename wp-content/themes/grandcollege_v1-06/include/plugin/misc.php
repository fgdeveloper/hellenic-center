<?php
	...
	*/
	...
	function get_vimeo($url, $width = 640, $height = 480){
		...
		<iframe src="http://player.vimeo.com/video/<?php echo $id[1]; ?>?title=0&amp;byline=0&amp;portrait=0" width="<?php echo $width; ?>" height="<?php echo $height; ?>" frameborder="0"></iframe>
	...
	function print_flex_slider($slider_xml, $size="980x360"){
		...
		foreach($slider_xml->childNodes as $slider){
			...
			if( !empty($title) ){
				echo '<div class="flex-caption"><div class="inner-caption gdl-slider-caption"><div class="gdl-slider-title gdl-title">' . $title . '</div>' . $caption . '</div></div>'; 
			}
		...
		foreach($slider_xml->childNodes as $slider){
			...
			if($link_type == 'Link to Video'){
				...
				if(!empty($title) || !empty($caption)){
					...
					echo '<div class="inner-caption gdl-slider-caption">';
