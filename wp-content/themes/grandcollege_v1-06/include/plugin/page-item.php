<?php
	...
	*/
	...
	function print_column_item($item_xml){
		...
		if(!empty($header)){
			...
			if( !empty( $dropcap_image ) ){
				echo '<div class="gdl-header-dropcap">';
				echo '<div class="gdl-header-dropcap-center">';
				echo '<img src="' . $dropcap_image[0] . '" class="no-preload" alt="" />';
				echo '</div>';
				echo '</div>';
			}		
			echo '<h3 class="column-header-title title-color gdl-title">' . $header . '</h3>';
	...
	// Print course item
	function print_course_item($item_xml){
		global $search_custom_meta;
		
		$category = find_xml_value($item_xml, 'category');
		$category = ($category == 'All')? '': $category;
		
		$course_posts = get_posts(array('post_type'=>'course', 'course-category'=>$category, 
			'numberposts'=>'100', 'meta_key'=>'course-option-id', 'orderby'=>'meta_value', 'order'=>'asc'));
		...
		foreach( $course_posts as $course_post ){
			...
			foreach ( $search_custom_meta as $custom_meta ){
				echo "<td>";
				if ( $custom_meta['show-on-table'] == 'Yes' ){
					if( !empty($custom_meta['link']) && $custom_meta['link'] == 'Yes' ){
	...
	// Print content slider
	function print_content_slider_item($item_xml){
		$header = find_xml_value($item_xml, 'header');
		if(!empty($header)){
			$dropcap_image = wp_get_attachment_image_src( find_xml_value($item_xml, 'header-icon') , 'full' );
			if( !empty( $dropcap_image ) ){
			...
			echo '<h3 class="content-slider-header-title title-color gdl-title">' . $header . '</h3>';
			echo '<div class="clear"></div>';
		}
		
		$content_node = find_xml_node($item_xml,'slider-content-item');
		echo '<div class="gdl-content-slider">';
		echo '<ul class="anythingSlider">';
		foreach( $content_node->childNodes as $content_child ){
			echo '<li>';
			echo '<div>';
			echo do_shortcode( $content_child->nodeValue );
			echo '</div>';
			echo '</li>';
		}
		echo '</ul>';
		echo '</div>'; // gdl-content-slider
		
	}

	if( $gdl_is_responsive ){
		$gallery_div_size_num_class = array(
			'1/4' => array( 'class'=>'four columns', 'size'=>'390x260', 'size2'=>'390x260', 'size3'=>'390x260'),
			'1/3' => array( 'class'=>'one-third column', 'size'=>'390x260', 'size2'=>'390x260', 'size3'=>'390x260'),
			'1/2' => array( 'class'=>'eight columns', 'size'=>'450x450', 'size2'=>'390x260', 'size3'=>'390x260'),
		); 	
	}else{
		$gallery_div_size_num_class = array(
			'1/4' => array( 'class'=>'four columns', 'size'=>'220x160', 'size2'=>'145x90', 'size3'=>'220x160'),
			'1/3' => array( 'class'=>'one-third column', 'size'=>'300x200', 'size2'=>'200x140', 'size3'=>'220x160'),
			'1/2' => array( 'class'=>'eight columns', 'size'=>'460x280', 'size2'=>'310x220', 'size3'=>'220x160'),
		); 			
	}
	...
	function print_search_course($item_xml){
		...
		if(!empty($header)){
			...
			echo '<h3 class="search-course-header-title title-color gdl-title">' . $header . '</h3>';
			echo '<div class="clear"></div>';
		}		
		
		echo '<form method="get" id="courseform" action="' . home_url() . '">';
		
		echo '<div class="search-attribute-wrapper">';
		echo '<div class="search-attribute-title gdl-title title-color">' . __('Search By Attribute', 'gdl_front_end') . '</div>';
		echo '<div class="search-attribute gdl-combobox">';
		
		echo get_search_course_combobox();
		?>	
			<div class="gdl-combobox-text"> Please Select Item
				<div class="gdl-combobox-button"></div>
			</div>
			<div class="clear"></div>
		<?php
		echo '</div>';
		...
		echo '<div class="fill-keyword-wrapper">';
		echo '<div class="fill-keyword-title gdl-title title-color">' . __('Fill Your Keywords','gdl_front_end') . '</div>';
		echo '<div class="fill-keyword">';
		?>
			<div id="search-course-text">
				<?php 	
					$default_value = __("Type your keywords...", "gdl_front_end");
				?>
				<input type="text" value="<?php echo $default_value; ?>" name="meta_value" autocomplete="off" data-default="<?php echo $default_value; ?>" />
				<input type="hidden" name="s" value="gdl-course" />
			</div>
			<input type="submit" id="search-course-submit" value="<?php _e("Search","gdl_front_end"); ?>"/>
			<br class="clear">
		<?php 
		echo '</div>';
		echo '</div>';
		...
		$text_description = find_xml_value($item_xml, 'text-description');
		if( !empty( $text_description ) ){
			echo '<div class="search-text-description">';
			echo do_shortcode( $text_description );
			echo '</div>';
		}
	...
	if( $gdl_is_responsive ){
		...
		$port_div_size_num_class = array(
			...
			"1/1" => array("class"=>"sixteen columns", "size"=>"630x225", "size2"=>"320x170", "size3"=>"190x135"));
	}
	$class_to_num = array(
		"element1-4" => 0.25,
		"1/4"=>0.25,
	...
	function print_page_item($item_xml){
		...
		while( have_posts() ){
			...
			// start printing data
			echo '<div class="' . $item_class . ' mb20 portfolio-item">'; 
