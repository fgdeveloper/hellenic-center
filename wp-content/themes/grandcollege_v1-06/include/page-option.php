<?php 
	...
	*/
	...
	$page_meta_boxes = array(
			...
			'name'=>array(
				...
				'Blog'=>array(
					...
					'item-size'=>array(
						...
						'options'=>array('0'=>'Widget Style', '1'=>'1/1 Full Thumbnail'),
						'type'=>'combobox',
				...
				'Course-Table'=>array(
					'category'=> array(
						'title'=> 'CATEGORY',
						'name'=>'page-option-item-course-table-category',
						'hr'=>'none',
						'type'=>'combobox'
					),
				),		
				...
				'Content-Slider' => array(
					'header'=>array(
						'title'=> 'HEADER TITLE',
						'name'=> 'page-option-item-content-slider-header-title',
						'type'=> 'inputtext',
						'hr'=> 'none'),			
					'header-icon'=>array(
						'title'=> 'HEADER ICON',
						'name'=> 'page-option-item-content-slider-header-icon',
						'type'=> 'upload'),		
					'slider-content-item'=>array(
						'caption'=>'page-option-item-content-slider-caption',
						'slider-content-num'=>'page-option-item-content-slider-num',
						'hr'=>'none'
					)
				),
				...
				'Personnal'=>array(
					...
					'header-icon'=>array(
						'title'=> 'HEADER ICON',
						'name'=> 'page-option-item-personnal-header-icon',
						'type'=> 'upload'),		
					'item-size'=>array(
						'title'=>'PERSONNAL SIZE',
						'name'=>'page-option-item-personnal-size',
						'options'=>array('0'=>'1/4', '1'=>'1/3', '2'=>'1/2', '3'=>'1/1 Full Width'),
						'type'=>'combobox'),
					'category'=>array(
						'title'=>'CHOOSE CATEGORY',
				...
				'Search-Course'=>array(
					'header'=>array(
						'title'=> 'HEADER TITLE',
						'name'=> 'page-option-item-search-course-title',
						'type'=> 'inputtext',
						'hr'=> 'none'),		
					'header-icon'=>array(
						'title'=> 'HEADER ICON',
						'name'=> 'page-option-item-search-course-icon',
						'type'=> 'upload'),
					'text-description'=>array(
						'title'=> 'TEXT DESCRIPTION',
						'name'=> 'page-option-item-search-text-description',
						'type'=> 'textarea'
					),
				),
		...
		"Open top slider"=>array('type'=>'open' ,'id'=>'open-top-slider-id','hr'=>'none'),
		...
		"Close top slider"=>array('type'=>'close','hr'=>'none'),
		
		"Under Slider" => array(
			'type'=>'combobox',
			'title'=> __('ENABLE UNDER SLIDER'),
			'options'=> array('Yes','No'),
			'hr'=>'none',
			'name'=>'page-option-enable-under-slider',
			'default'=>'No'
		),
		
		"Open under slider"=>array('type'=>'open' ,'id'=>'open-under-slider-id','hr'=>'none'),
		
		"Under Slider Left"=>array(
			'type'=>'textarea',
			'title'=> __('UNDER TITLE LEFT'),
			'hr'=>'none',
			'name'=>'page-option-under-slider-left'
		),
		
		"Under Slider Right"=>array(
			'type'=>'textarea',
			'title'=> __('UNDER TITLE RIGHT'),
			'hr'=>'none',
			'name'=>'page-option-under-slider-right'		
		),
		
		"Close under slider"=>array('type'=>'close','hr'=>'none'),
	);
	...
	function add_page_option_element(){
		...
		$page_meta_boxes['Page Item']['name']['Course-Table']['category']['options'] = get_category_list( 'course-category' );
			...
			if( $edit_meta_box['type'] == 'page-option-item' ){
				...
				for($i=0; $i<$num; $i++){
					...
					if(!isset($item_content_num[$item_type_new])){
						...
						}else if($item_type_new == 'Content-Slider'){
						
							$item_content_num['slider-content-item'] = 0;
					...
					foreach($item_content as $key => $value){
						...
						}else if($key == "slider-content-item"){
						
							$item_xml = $item_xml . '<' . $key . '>';
							
							$content_slider_num = $_POST[$value['slider-content-num']][$item_content_num[$item_type_new]];
							for($j=0; $j<$content_slider_num; $j++){
							   
								$temp = isset( $_POST[$value['caption']][$item_content_num['slider-content-item']] )? htmlspecialchars($_POST[$value['caption']][$item_content_num['slider-content-item']]) : '';
								$item_xml = $item_xml . create_xml_tag('caption', $temp);
								
								$item_content_num['slider-content-item']++; 
		...
		?>
			...
			<?php
				...
				foreach( $name[$item_type] as $input_key => $input_value ){
					...
					}else if( $input_key == 'slider-content-item' ){
						
						print_box_content($input_value, find_xml_node($xml_val, 'slider-content-item'));
	...
	function print_box_content($name, $values){

		?>
		
		<div class="meta-body">
			<div class="meta-title meta-tab">ADD MORE CONTENTS</div>
			<div id="page-content-slider-add-more" class="page-content-slider-add-more" /></div>
			<br class="clear">
			<div class="meta-input">
				<input type='hidden' class="content-slider-num" id="content-slider-num" name='<?php echo $name['slider-content-num']; ?>[]' value=<?php 
					
					echo empty($values)? 0: $values->childNodes->length;

				?> />
				<div class="added-content-slider" id="added-content-slider">
					<ul>
						<li id="page-item-content-slider" class="default">	
							<div class="meta-title meta-tab-title">TABS TEXT</div><textarea id='<?php echo $name['caption']; ?>' ></textarea> <br>
							<div id="unpick-content-slider" class="unpick-content-slider"></div>
						</li>
						...
						<?php
							...
							?>
								<li id="page-item-content-slider" class="page-item-content-slider">	
									<div class="meta-title meta-tab-title">TABS TEXT</div><textarea name='<?php echo $name['caption']; ?>[]' id='<?php echo $name['caption']; ?>' ><?php echo $tab->nodeValue; ?></textarea> <br>
									<div id="unpick-content-slider" class="unpick-content-slider"></div>
								</li>			
							<?php
