...
if(!function_exists("get_the_author_posts_link")){
	function get_the_author_posts_link(){
		return '<a href="' . get_author_posts_url(get_the_author_meta( 'ID' )) . '" title="' . esc_attr( sprintf(__("Visit %s&#8217;s all posts"), get_the_author()) ) . '" rel="author">' . get_the_author() . '</a>';
	}
}
...
function theme_get_image_src($source, $size = 'full', $quality=''){
	...
	$timthumb = theme_get_option('advance', 'timthumb');
	if($timthumb){
...
class ThemeImageResizer {
	...
	protected function get_file_basename($file){
...
class ImageResizerByAttachmentId extends ThemeImageResizer {
	...
	protected function get_file_basename($file){
		return $this->attachment_id.'_'.wp_basename($file);
	}
	...
	protected function resize(){
		if(stripos($this->attachment_id,'ngg-') !== false && class_exists('nggdb')) {
			...
			if ( !preg_match('!^image/!', get_post_mime_type( $this->attachment_id ))) {
				return new WP_Error('attachment_is_not_image', __( 'Attachment is not image' ));
			}
		...
		if ( empty($info) || !in_array($info[2], array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG))) // only gif, jpeg and png images can reliably be displayed
			return new WP_Error('image_type_is_not_correctly', __( 'Image type is not correctly' ));
...
class ImageResizerByUrl extends ThemeImageResizer {
	...
	public function resize(){
		...
		if(!is_file($file)){
			return new WP_Error('file_is_not_exists', __( 'File is not exists' ));
		}
		$resized_file = $this->resize_process($file, $this->width, $this->height,$this->size_name,$this->cache_dir,$this->quality);
...
function theme_add_cufon_code(){
	$code = stripslashes(theme_get_option('cufon','code'));
	$fonts = theme_get_option('cufon','fonts');
	
	if(trim($code) == '' && !empty($fonts)){
		$first = array_shift($fonts);
		if(is_array($first)){
			$font_name = $first['font_name'];
		}else{
			$file_content = file_get_contents(THEME_FONT_DIR.'/'.$first);
...
function theme_exclude_from_search(){
	...
	$post_types = theme_get_option('advance','exclude_from_search');
	if(!empty($post_types)){
