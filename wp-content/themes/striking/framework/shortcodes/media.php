...
function theme_video_youtube($atts, $content=null) {
	...
	if (!$hd){
		$hd = (theme_get_option('media','youtube_hd') == true)?1:0;
	}else{
		if($hd==='true'){
			$hd = '1';
		}else{
	...
	if (!$showsearch){
		$showsearch = (theme_get_option('media','youtube_showsearch') == true)?1:0;
	}else{
		if($showsearch==='true'){
			$showsearch = '1';
		}else{
	...
	if (!empty($clip_id)){
		return "<div class='video_frame'><iframe class='youtube' style='height:{$height}px;width:{$width}px' src='http://www.youtube.com/embed/{$clip_id}?autohide={$autohide}{$autoplay}&amp;controls={$controls}&amp;disablekb={$disablekb}&amp;fs={$fs}&amp;hd={$hd}&amp;loop={$loop}&amp;rel={$rel}&amp;showinfo={$showinfo}&amp;showsearch={$showsearch}&amp;wmode=transparent' width='{$width}' height='{$height}' frameborder='0'></iframe></div>";
	}
}
