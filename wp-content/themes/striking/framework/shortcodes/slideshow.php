...
function theme_slideshow_nivo($atts, $content = null){
	...
	if(!empty($source)){
		...
		$images = theme_generator('slideShow_getImages',$source,$number,'full');
		foreach($images as $image) {
			if($image['link'] != false){
				...
				default:
					...
					if($image['link'] != false){
						if($caption_position != '' && $caption_position !='disable'){
							$content .=  '<a href="'.$image['link'].'" target="'.$image['target'].'" class="anything_caption caption_'.$caption_position.'">';
						...
						if($caption_position != '' && $caption_position !='disable'){
							$content .=  '<div class="anything_caption caption_'.$caption_position.'">';
							$content .=  '<h3>'.$image['title'].'</h3>';
							if($image['desc']){
								$content .=  '<p>'.$image['desc'].'</p>';
							}
